...
每個「學段_科目_年級」一個 JSON 檔，共 116 檔、每檔 20 題
（難度 ⭐7 題、⭐⭐7 題、⭐⭐⭐6 題），合計 2,320 題。
（2026-07-10 第二版：大學由 7 科擴充為 8 大學群 46 科。）
...
選學段 → 選科目年級 → 選挑戰模式 → 作答 → 交卷自動改分 → 成績＋逐題詳解。

三種模式：
- `normal` 一般練習：自選難度與題數，可前後翻題。
- `rush` 搶答挑戰：每題限時 15 秒（剩 5 秒倒數變紅），逾時 answer=null 判錯並自動換題；
  作答後立即揭曉正解（綠）與誤答（紅），連對數 streak 累計，成績頁顯示最高連對。
- `stage` 闖關模式：第 n 關 = 難度 n，每關 5 題答對 ≥4 題晉級，否則只能重打本關；破三關為通關。

加值：錯題本（可重考、答對即移除）、計時器、成績紀錄、一鍵分享、深色模式、
題目與選項隨機排序、優先抽出 localStorage 未記錄過的題目（`tb_seen`），
題庫檔以 `?v={manifest.generated}-{total}` 破快取，題庫更新後使用者立即拿到新題。
